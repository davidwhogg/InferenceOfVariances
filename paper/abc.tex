...
like sigma-8.
Because we have set $N=5$, we do not have a high signal-to-noise
...
computation (ABC)\footnote{See, for example, \url{https://en.wikipedia.org/wiki/Approximate_Bayesian_computation}.
  ...
  \cite{ishida}.}, a simple version of which we introduce now.
...
All this means that it is likely that there \emph{are} tractable ABC
methods for cosmology.\footnote{Indeed, some already exist, for
  example the previously cited \cite{ishida}, and work by
  our own group---Hahn, Vakili, \etal (in preparation).}

\textbf{Take-home message 3:} \emph{There are likely to be tractable
  versions of these ideas for large-scale structure inferences.}
