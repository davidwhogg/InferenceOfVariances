...
This made-up pseudo-likelihood is patently absurd:
...
\textbf{Take-home message 0:} \emph{When we infer the correlation function
(what should be called the covariance function) of the density field,
let's only consider functions that obey Mercer's condition!}
(After all, we would never consider $V<0$ in the trivial problem!)
This is much easier to do in Fourier space than real space.
That isn't an argument for doing the \emph{analysis} in Fourier space,
but it is an argument for representing the function $\xi(r)$ in
Fourier space.
HOGG: This point is slightly off-topic here, because in the context of the
Bayesian inference, this will appear in the prior, not the likelihood,
and up to now we have only been discussing the likelihood.
...
HOGG: Return to the LSS problem and discuss three things: What the
RTTD is. How, in detail, cosmologists get the $\Sigma$ horror. What
ABC looks like. CITE HAHN AND VAKILI.
