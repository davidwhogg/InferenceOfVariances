...
sampling.
In the limit that the distance threshold is very small, ABC returns a
posterior sampling.

\figurename~\ref{fig:abc} shows the ABC-generated posterior for a small
threshold (set to 1/16 in this case).
The distance metric $\Delta^2$ makes no reference to means, so it doesn't
constrain the mean $\mu$.
However, the ABC results for the variance $V$ are indistinguisable
from the true posterior sampling shown in
\figurename~\ref{fig:correct}.%
\begin{figure}
